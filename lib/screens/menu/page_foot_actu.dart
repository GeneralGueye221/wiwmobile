...
import 'package:wiwmedia_wiwsport_v3/model/menu/category_foot.dart';
...
import 'package:wiwmedia_wiwsport_v3/screens/menu/LandingPages/foot.dart';

import 'package:wiwmedia_wiwsport_v3/screens/menu/page_newsletter.dart';

import '../../MyDrawer.dart';

class PageFootActu extends StatefulWidget {
  final int index;
  bool isUser;
  Utilisateur user;
  PageFootActu({this.index, this.isUser, this.user});
  @override
  _PageFootActuState createState() => _PageFootActuState();
}

class _PageFootActuState extends State<PageFootActu> {
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
            ...
            bottom: PreferredSize(
              ...
              child: TabBar(
                ...
                onTap: (int) {
                  Navigator.push(context, MaterialPageRoute(builder: (context)=> PageFootActu(user: widget.user, isUser: widget.isUser, index: int)));
                },    
                tabs: List.generate(
                    category_foot.length,
                    (index) => Text(
                          category_foot[index].nom,
                        )),
              ),
            ),
            backgroundColor:
          ...
          body: Stack(
            ...
            children: [
              TabBarView(
                children: [
                  LandingFoot(user: widget.user,tab_value: 0),
                  LandingFoot(user: widget.user,tab_value: 1),
                  LandingFoot(user: widget.user,tab_value: 2),
                  LandingFoot(user: widget.user,tab_value: 3),
                  LandingFoot(user: widget.user,tab_value: 4),
                  LandingFoot(user: widget.user,tab_value: 5),
                  LandingFoot(user: widget.user,tab_value: 6),
                  LandingFoot(user: widget.user,tab_value: 7),
                  LandingFoot(user: widget.user,tab_value: 8),
                  LandingFoot(user: widget.user,tab_value: 9),
                  LandingFoot(user: widget.user,tab_value: 10),
                ],
              ),
              Container(
                  height: 75,
